...
def build_turn_content(g, player_line):
    ...
    open_quests = [q["text"] for q in g.get("quests", []) if not q.get("done")]
    quests_str = "\n".join(f"- {q}" for q in open_quests) or "(none)"
    npcs_str = "\n".join(f"- {n}: {note}" if note else f"- {n}" for n, note in g.get("npcs", {}).items()) or "(none)"
...
def load_game(cid):
    """Load a channel's game and backfill any missing schema keys."""
    return game.ensure_keys(db.load_game(cid, game.fresh_game))
...
def apply_tags(g, quests, npcs):
    """Fold auto-detected quests/NPCs into game state. Returns confirmation lines."""
    notes = []
    existing = {q["text"].lower() for q in g["quests"]}
    for q in quests:
        if q.lower() not in existing:
            g["quests"].append({"text": q, "done": False})
            notes.append(f"📜 New quest logged: *{q}*")
    for nm, note in npcs:
        if nm and nm not in g["npcs"]:
            notes.append(f"🧑 Met **{nm}**" + (f" — {note}" if note else ""))
        if nm:
...
class SheetView(discord.ui.View):
    ...
    async def long_rest(self, interaction, button):
        async with get_lock(interaction.channel_id):
            g = load_game(interaction.channel_id)
...
HELP_TEXT = """__**Odrun Fell — how to play**__
Just type what your character does in **#{ch}** and the AI DM responds. Most commands also work as `/slash`.

**🎲 Rolls**
`!roll 1d20+3` — roll dice
`!check <name> <skill/ability>` — e.g. `!check Bungua stealth` (proficiency added automatically)

**📜 Your character**
`!sheet [name]` · `!spells <name>` · `!rest <name>` (long rest)
`!create <name> <race> <class>` — roll up a new character (or `/create` for menus)

**❤️ Stats** (usually the DM tells you when to use these)
`!damage <name> <n>` · `!heal <name> <n>` · `!slot <name>` (cast) · `!deathsave <name>` (when at 0 HP)
`!gold <name> +N` · `!xp <name> +N` (auto-levels) · `!give/!drop <name> <item>` · `!condition <name> add|remove <x>`

**⚔️ Combat**
`!spawn <monster>` · `!monsters` · `!attack <pc> <monster>` · `!mattack <monster> <pc>`
`!init start` → `!init roll <name> <mod>` → `!init go` → `!next` (turn order)

**🗺️ World**
`!recap` — where you left off · `!quests` · `!npcs` · `!map`
`!scene <description>` — paint a picture of the scene (if art is on)

Quests, NPCs, and area art are tracked **automatically** as you play — the commands above are just for viewing or manual tweaks.
Monsters: goblin, wolf, skeleton, bandit, zombie, giant spider, cultist, bone-gnawer, ichor-hound, relic-wight."""
...
async def _process_message(message):
    ...
    if lower == "!recap":
        open_q = [q["text"] for q in g["quests"] if not q.get("done")]
        party = " · ".join(f"{n} {c['hp']}/{c['max_hp']}hp" for n, c in g["characters"].items()) or "(none)"
        quests = "\n".join(f"• {q}" for q in open_q) or "(none)"
        npcs = "\n".join(f"• {n}" + (f" — {note}" if note else "") for n, note in g["npcs"].items()) or "(none)"
        pins = "\n".join(f"📌 {p}" for p in g["pinned"]) or "(none)"
        await ch.send(
            f"__**Where you left off**__\n\n**Story so far:**\n{g['summary'] or '(just getting started)'}\n\n"
            f"**Open quests:**\n{quests}\n\n**People you've met:**\n{npcs}\n\n"
            f"**Party:** {party}\n\n**Pinned facts:**\n{pins}"[:1950])
        return
    ...
    if lower.startswith("!quest"):
        if lower == "!quests" or (len(parts) == 1):
            if not g["quests"]:
                await ch.send("No quests yet."); return
            lines = []
            for i, q in enumerate(g["quests"], 1):
                lines.append(f"{i}. {'✅' if q.get('done') else '⬜'} {q['text']}")
            await ch.send("**Quest log**\n" + "\n".join(lines)); return
        sub = parts[1].lower()
        if sub == "add" and len(parts) > 2:
            g["quests"].append({"text": " ".join(parts[2:]), "done": False})
            await send_save("📜 Quest added."); return
        if sub in ("done", "complete") and len(parts) > 2 and parts[2].isdigit():
            i = int(parts[2]) - 1
            if 0 <= i < len(g["quests"]):
                g["quests"][i]["done"] = True
                await send_save(f"✅ Completed: *{g['quests'][i]['text']}*"); return
            await ch.send("No quest with that number."); return
        if sub in ("remove", "rm", "delete") and len(parts) > 2 and parts[2].isdigit():
            i = int(parts[2]) - 1
            if 0 <= i < len(g["quests"]):
                removed = g["quests"].pop(i)
                await send_save(f"🗑️ Removed: *{removed['text']}*"); return
            await ch.send("No quest with that number."); return
        await ch.send("`!quests` · `!quest add <text>` · `!quest done <n>` · `!quest remove <n>`"); return

    # ---- npcs ----
    if lower.startswith("!npc"):
        if lower == "!npcs" or len(parts) == 1:
            if not g["npcs"]:
                await ch.send("No NPCs recorded yet."); return
            await ch.send("**People you've met**\n" + "\n".join(
                f"• **{n}**" + (f" — {note}" if note else "") for n, note in g["npcs"].items())); return
        name = parts[1]
        if len(parts) == 2:  # show one
            note = g["npcs"].get(name)
            await ch.send(f"**{name}** — {note or '(no notes)'}" if name in g["npcs"] else f"No record of '{name}'."); return
        g["npcs"][name] = " ".join(parts[2:])  # set/update note
        await send_save(f"🧑 Noted **{name}** — {g['npcs'][name]}"); return
    ...
    # ---- art (manual; bypasses the auto-scene cooldown so it always fires) ----
...
async def s_damage(interaction: discord.Interaction, name: str, amount: int):
    async with get_lock(interaction.channel_id):
        g = load_game(interaction.channel_id); cn = game.find_char(g, name)
        if not cn:
            await interaction.response.send_message("No such character.", ephemeral=True); return
